...
-  随机森林(Random
   ...
   tree)建模，然后将这些决策树组合在一起，通过对所有决策树结果的平均(Mean)或投票(Vote)得出最终预测的回归(Regression)或分类(Classification)的结果。
...
1. :math:`m`: Bootstraping抽样的样本数量；

2. :math:`n\_tree`: 构建的决策树数量；

**随机森林构建大致过程：**

1. 从原始训练数据集中使用Bootstraping方法随机有放回采样取出\ :math:`M`\ 个样本，共进行\ :math:`n\_tree`\ 次采样，生成\ :math:`n\_tree`\ 个训练集；

2. 对\ :math:`n\_tree`\ 个训练集，分别训练\ :math:`n\_tree`\ 个决策树模型；

3. 对于单个决策树模型，假设训练样本特征的个数为\ :math:`n`\ ，那么每次分裂时根据\ ``信息增益/信息增益率/基尼指数``\ 选择最好的特征进行分裂；

4. 每棵树都一直这样分裂下去，直到该节点的所有训练样本都属于同一类。在决策树的分裂过程中不需要剪枝；

5. 将生成的多棵决策树组成随机森林。对于分类问题，按照多棵树分类器投票决定最终分类结果；对于回归问题，由多克树预测值的均值决定最终预测结果；

6. 注意：OOB(out-of-bag)：每棵决策树的生成都需要自助采样，这时就有\ :math:`\frac{1}{3}`\ 的数据未被选中，这部分数据就称为\ **袋外数据**\ ；
...
**(1)
每棵决策树都对应一个训练集数据，要构建\ :math:`n\_tree`\ 棵决策树，就需要产生对应数量(\ :math:`n\_tree`)的训练集，从原始训练集中产生\ :math:`n\_tree`\ 个训练子集要用到统计抽样技术。现有的统计抽样技术很多，按照抽样是否放回主要包括以下两种：**
...
-  (ii)放回抽样
   ...
   -  无权重放回抽样(Bootstrap抽样)
      ...
      -  无权重抽样，也叫bagging方法。是一种用来提高学习算法准确度的方法。该方法于1996年由Breiman根据Boosting技术提出的。bagging方法是以可重复的随机抽样为基础的，每个样本是初始数据集有放回抽样。在可重复抽样生成多个训练子集时，存在于初始训练集D中的所有的样本都有可能被抽取的可能，但在重复多次后，总有一些样本是不能被抽取的，每个样本不能被抽取的概率为\ :math:`(1-\frac{1}{N})^N`\ 。

   -  有权重放回抽样

      -  有权重抽样，也叫boosting方法，也叫更新权重抽样。Boosting方法抽样，首先有放回随机抽样产生一组(\ :math:`n \leqslant N`)训练集，然后对这组训练集中每一个训练集设定权重为\ :math:`\frac{1}{n}`\ ，在设定权重后，对每个带权重的训练集进行测试(决策树训练)，在每次测试结束后，对分类性能差的训练集的权重进行提升，从而产生一个新的权重系列，经过多次训练后，每个训练集就有一个和其对应的权重，在投票时，这些权重就可以对投票的结果产生影响。从而影响最终的决策结果。

**(2)
Bagging和Boosting方法都是可放回的抽样方法，但两者间存在很大的差别：**
...
**(3)
随机森林算法在生成的过程中，主要采用bagging方法，也就是Bootstrap抽样。**

-  从原始训练集中产生M个训练子集，每个训练子集的大小约为原始训练集的\ :math:`\frac{2}{3}`\ ，每次抽样均为随机且放回抽样，这样使得训练子集中的样本存在一定的重复，这样做的目的是为了使得随机森林中的决策树不至于产生局部最优解。
...
-  `决策树 <http://rpubs.com/Wangzf/CART>` 已经介绍了这一部分内容，随机森林常用的主要有C4.5,
   CART.
...
-  由于随机森林在节点分裂时，不是所有的属性(变量)都参与属性(变量)指标的计算，而是随机地选择某几个属性(变量)参与比较，参与的属性个数就称之为随机特征变量。随机特征变量是为了使每棵决策树之间的相关性减少，同时提升每棵决策树的分类精度，从而提升整个随机森林的性能而引入的。其基本思想是，在进行节点分裂时，让所有的属性(变量)按照某种概率分布随机选择其中某几个属性参与节点分裂过程。在随机森林算法中，随机特征变量的产生方法主要有两种：
   ...
   -  **随机选择输入变量(Forest-RI)**
      ...
      -  Forest-RI是对输入变量(p个)随机分组(每组变量的个数F是一个定值)，然后对于每组变量，利用CART方法产生一棵树，并让其充分生长，不进行剪枝。在每个节点上，对输入该节点的变量，重复前面的随机分组，再重复CART方法，直到将所有节点均为叶节点为止。一般F有两种选择，首先是F=1，其次取F为小于\ :math:`log_{2}(p+1)`\ 的最大整数。假如只有很少的输入变量，比如p值不大，用Forest-RI法从p中随机选择F个作为随机特征变量，这样可能提高每棵树模型的精度，但同时也增大了各棵树之间的相关系数。

   -  **随机组合输入变量(Forest-RC)**

      -  Forest-RC是先将随机特征进行线性组合，然后再作为输入变量来构造随机森林的方法。

-  最常用的随机森林算法都是使用Forest-RI方法构建，在每棵子树的生长过程中，不是将全部p个输入变量参与节点分裂，而是随机抽取指定F(\ :math:`F \leqslant p`)个随机特征变量，F的取值一般为\ :math:`log_{2}(p+1)`\ ，以这F个属性上最好的分裂方式对节点进行分裂，从而达到节点分裂的随机性。
...
3. 用新的特征集重复上述过程，直到剩下\ :math:`m`\ 个特征(\ :math:`m`\ 为提前设定的值)；
...
2. 通过\ ``permutation``\ 的方式将原来的所有\ :math:`N`\ 个样本的第\ :math:`i`\ 个特征重新打乱分布；

第二种方法更加科学，保证了特征替代值与原特征的分布是近似的。这种方法叫做permutation
test ，即在计算第i个特征的重要性的时候，将N
个特征的第i个特征重新洗牌，然后比较D和表现的差异性，如果差异很大，则表明第i个特征是重要的。
...
1. 随机森林泛化误差的收敛性
   ...
   -  随机森林中的决策树的泛化误差都收敛于：\ :math:`\underset{n \rightarrow \infty}{\lim}PE^{*}=P_{xy}(P_{\Theta}(k(X,\Theta)=Y)-\underset{j\neq Y}{\max}P_{\Theta}(k(X,\Theta)\neq Y) > 0)`

   -  随着随机森林中决策树数量(\ :math:`n`)的增加，随机森林泛化误差(\ :math:`PE^{*}`)将趋向一个上界。随机森林对未知实例有很好的扩展性，也就是说随机森林随着决策树数量的增多不易过拟合

2. 随机森林中决策树的相关度和强度影响算法的泛化误差

   -  随机森林泛化误差的上界为：\ :math:`PE^{*}\leqslant \frac{\bar{\rho}(1-s^{2})}{s^{2}}`

      -  :math:`\bar{\rho}`\ 为决策树之间的相关度的平均值

      -  :math:`s`\ 为决策树的平均强度

   -  要使随机森林的泛化性能好，则应该尽量减小决策树之间的相关性(\ :math:`\rho`)，增大单棵树的分类性能(\ :math:`s`)。每棵树的分类强度越大，则随机森林的分类性能越好，森林中树之间的相关度越小，则随机森林的分类性能越好
...
-  **衡量随机森林性能的指标：**
   ...
   -  分类效果
      ...
      -  分类精度:准确度(acccuracy of measurement),
         是指使用算法得出的分类结果与真实之间的接近程度
      ...
      -  二分类数据的混淆矩阵
         ...
         -  灵敏度(Sensitivity)(正类的的分类精度)：\ :math:`Sensitivity=\frac{TP}{TP+FN}`

         -  特异度(Specificity)(负类的的分类精度)：\ :math:`Specificity=\frac{TN}{FP+TN}`

         -  几何均值(G-mean)：\ :math:`G-mean=\sqrt{\frac{TP}{TP+FN}\times \frac{TN}{FP+TN}}`

         -  负类的查全率(Recall)：\ :math:`Recall=\frac{TP}{TP+FN}`

         -  负类的查准率(Precision)：\ :math:`Precision=\frac{TP}{TP+FP}`

         -  负类的检验值(F-value)：\ :math:`F-value=\frac{(1+\beta^{2})\cdot recall \cdot precision}{\beta^{2}\cdot recall \cdot precision}, \beta \in (0, 1]`
      ...
      -  泛化误差(generalization error)
         ...
         -  估计泛化误差：
            ...
            -  OOB估计
               ...
               -  随机森林是使用Bootstrap来进行每棵树训练集的生成，在生成这些(M)个训练集时，初始训练集中有一些样本是不能被抽取的这些样本的个数是初始数据集的\ :math:`(1-\frac{1}{N})^N`\ 。可以证明，当\ :math:`N`\ 足够大时，\ :math:`(1-\frac{1}{N})^N`\ 将收敛于\ :math:`\frac{1}{e}\approx 0.368`\ ，说明将有近\ :math:`37\%`\ 的样本不会被抽取出来，这些不能被抽取的样本组成的集合，称之为袋外数据(OOB)。
