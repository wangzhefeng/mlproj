...
-  集成学习根据各个弱分类器之间有无依赖关系，分为 Boosting 和 Bagging
   ...
   -  Boosting
      ...
      Boosting Decision Tree)、Xgboost、LightGBM、CatBoost等；
...
Adaboost 是 boosting 流派中最具代表性的一种方法。AdaBoost，是英文
"Adaptive Boosting"(自适应增强)的缩写，由 Yoav Freund 和 Robert Schapire
在1995年提出。它的自适应在于：前一个基本分类器分错的样本会得到加强，加权后的全体样本再次被用来训练下一个基本分类器。同时，在每一轮中加入一个新的弱分类器，直到达到某个预定的足够小的错误率或达到预先指定的最大迭代次数。

最初提出的 AdaBoost 算法主要用来解决二分类问题:

-  首先, 初始化所有的训练数据样本的权重为
   :math:`\omega_{i}=1/N, i=1, 2, \ldots, N`,
   并利用这个加权重的训练数据训练产生一个弱分类器.

-  然后, 在算法第 :math:`m` 步迭代中, :math:`m=2, 3, \ldots, M`,
   算法每次都会根据前一步训练出的弱分类器在训练数据上的分类结果重新计算训练样本权重,
   并在每次拟合之前将权重
   :math:`\omega_{1}, \omega_{2}, \ldots, \omega_{N}`
   作用在每个训练数据观测值 :math:`(x_{i}, y_{i}), i = 1, 2, \ldots, N`
   上, 然后不断将弱分类算法应用在这些加权之后的训练数据上,
   重新进行拟合分类. 那些在前一步迭代中被弱分类器
   :math:`G_{m-1}(\mathbf{x})` 误分类的的观测值的权重将会增大,
...
learner) :math:`G_{m}(\mathbf{x}), m=1, 2, \ldots, M` 时会使得 AdaBoost
...
**AdaBoost 具体算法：**
   ...
   1. 初始化每个样本观测值的权重 :math:`\omega_{i}^{[0]}=1/N` , 其中
      :math:`i=1, \ldots, N`\ ；

   2. 开始迭代, 令 :math:`m=1`

   -  利用加权重 :math:`\omega_{i}^{[m-1]}` 的训练数据拟合一个弱分类器
      :math:`G_{m}(\cdot)`\ ；

   -  计算加权训练数据的分类错误率:
      :math:`err^{[m]} = \frac{\sum_{i=1}^{N}\omega_{i}^{[m-1]}I(y_{i} \neq G_{m}(x_{i}))}{\sum_{i=1}^{N}\omega_{i}^{[m-1]}}.`

   -  根据上一步的分类错误率计算分类器在最终分类结果上的权重值:
      :math:`\alpha^{[m]}=log(\frac{1-err^{[m]}}{err^{[m]}}).`

   -  根据上面计算出的样本的误分类率更新样本的权重:
      :math:`\omega_{i}^{[m]} \leftarrow \omega_{i}^{[m-1]}exp(\alpha^{[m]}I(y_{i} \neq G_{m}(\mathbf{x}_{i})))`

   1. 重复上面的迭代，直到满足迭代停止条件 :math:`m=M`,
      并且将所有在迭代中产生的弱分类器通过加权投票的方法进行聚合,
      最终得到的强分类器为:
      :math:`G(\mathbf{x}) = sign\Bigg(\sum^{M}_{m=1}\alpha^{[m]}G^{[m]}(\mathbf{x})\Bigg)`
