...
def RSE(pred, true):
    ...
    """
    return np.sqrt(
        np.sum((true - pred) ** 2)) / np.sqrt(np.sum((true - true.mean()) ** 2)
    )
...
def main():
    ...
    real_dt = pd.DataFrame({
        'timestamp': timestamp[100:] + np.random.random(1000)/10, 
        'value': rdm
    })

    predict_dt = {}
    predict_dt.update({
        'timestamp': timestamp + np.random.random(1100)/10
    })
    for i in range(10):
        j = f'value{i}'
    ...
    # 检查数据有效性, 输出以 validate_idx 为索引的有效数据
    valid_data = PredictValidation(real_dt, predict_dt, p_interval, validate_idx=[1, 3, 5, 15, 20, 100, 0, -1, -2])
