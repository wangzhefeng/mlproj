...
# Pre-Trained model
# ------------------------------
from MobileNet_v3_pretrain import net as mobilenet
...
# ------------------------------
# img preprocess
mobile_preprocess_img = MobileNet_V3_Small_Weights.DEFAULT.transforms()
panda_preprocess = mobile_preprocess_img(panda_int).unsqueeze(dim = 0)
print(f"Input Image shape: {panda_preprocess.shape}")

# img predict
panda_preds2 = mobilenet(panda_preprocess)
print(f"Output Image shape: {panda_preds2.shape}")

# target labels
cates_mobile = MobileNet_V3_Small_Weights.DEFAULT.meta["categories"]
pred = [cates_mobile[idx] for idx in panda_preds2.argsort()[0].numpy()][::-1][:3]
print(pred)
